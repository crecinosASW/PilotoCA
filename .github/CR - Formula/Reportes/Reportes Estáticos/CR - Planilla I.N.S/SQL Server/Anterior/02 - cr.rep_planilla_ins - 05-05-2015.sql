...
FROM (	
	SELECT 1 orden,
		...
		LEFT(ISNULL(cia_fax, '') + REPLICATE(' ', 8), 8) + --teléfono
		LEFT(ISNULL(cia_telefonos, '') + REPLICATE(' ', 8), 8) +
	...
	SELECT 4 orden,
		...
		LEFT(REPLACE(ISNULL(ide_isss, ''), '-', '0') + REPLICATE(' ', 25), 25) + --no. asegurado
		...
		RIGHT(REPLICATE('0', 10) + REPLACE(CONVERT(VARCHAR, ISNULL((
