IF EXISTS (SELECT NULL
		   FROM sys.objects
		   WHERE object_id = object_id('cr.rep_planilla_mensual'))
BEGIN
	DROP PROCEDURE cr.rep_planilla_mensual
END
...
----------------------------------------------------------------------------------
-- evolution                                                                    --
-- reporte de CR - Planilla Mensual con anticipo quincenal - Costa Rica               --
----------------------------------------------------------------------------------
--EXEC cr.rep_planilla_mensual 5, '1', '20140802', NULL, NULL, 'jcsoria'
CREATE PROCEDURE cr.rep_planilla_mensual (
   @codcia INT = NULL,
...
SELECT ROW_NUMBER() OVER(PARTITION BY hpa_nombre_areafun, hpa_nombre_unidad ORDER BY hpa_nombre_areafun, hpa_nombre_unidad, hpa_codemp ASC) contador,
	...
	ISNULL((
			...
			WHERE inn_codppl = hpa_codppl
				...
				AND inn_codtig IN (SELECT iag_codtig
								   FROM sal.iag_ingresos_agrupador
								   WHERE iag_codagr = @codagr_anticipo)), 0.00) anticipo,	
	ISNULL((
			SELECT SUM(inn_valor)
			FROM sal.inn_ingresos
			WHERE inn_codppl = hpa_codppl
