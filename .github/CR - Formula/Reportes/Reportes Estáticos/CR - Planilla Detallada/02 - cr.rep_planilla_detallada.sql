...
SELECT cia_descripcion,
	...
	ISNULL((
		SELECT TOP(1) ISNULL(cco_cta_contable, '')
		FROM eor.cpp_centros_costo_plaza
			LEFT JOIN eor.cco_centros_de_costo ON cpp_codcco = cco_codigo		
		WHERE cpp_codplz = hpa_codplz), '') cco_cta_contable,
	ISNULL((
		SELECT TOP(1) ISNULL(cco_descripcion, '')
		FROM eor.cpp_centros_costo_plaza
			LEFT JOIN eor.cco_centros_de_costo ON cpp_codcco = cco_codigo		
		WHERE cpp_codplz = hpa_codplz), '') cco_descripcion,
	hpa_codplz,
	hpa_nombre_plaza,
	hpa_coduni,
	hpa_nombre_unidad,
