...
--EXEC cr.GenPla_Poliza_Contable 1124
...
BEGIN
    ...
    SELECT inn_codppl AS dco_codppl, 
	       ...
	       round(case when inn_codmon = 'USD' then sum(inn_valor) * hpa_tasa_cambio else sum(inn_valor) end * cpp_porcentaje / 100.00, 3)  AS dco_debitos,
	       0.00 AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   round(case when inn_codmon = 'USD' then sum(inn_valor) else sum(inn_valor) / hpa_tasa_cambio end * cpp_porcentaje / 100.00, 3)  AS dco_debitos_usd,
    ...
    GROUP BY inn_codppl, tig_cuenta, cco_nomenclatura_contable, tig_descripcion, cco_descripcion, inn_codmon, hpa_tasa_cambio, cpp_porcentaje
    ...
    SELECT res_codppl AS dco_codppl, 
	       ...
	       round(case when res_codmon = 'USD' then sum(res_valor) * hpa_tasa_cambio else sum(res_valor) end * cpp_porcentaje / 100.00, 3)  AS dco_debitos,
	       0.00 AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   round(case when res_codmon = 'USD' then sum(res_valor) else sum(res_valor) / hpa_tasa_cambio end * cpp_porcentaje / 100.00, 3)  AS dco_debitos_usd,
	       0.00 AS dco_creditos_usd,
	       @anio dco_anio,
	       @mes dco_mes,
    ...
    GROUP BY res_codppl, trs_cuenta, trs_codigo, cco_codigo,  trs_descripcion, cco_descripcion, cco_nomenclatura_contable, res_codmon, hpa_tasa_cambio, cpp_porcentaje
	...
	FROM (
		SELECT res_codppl AS dco_codppl, 
			   ...
			   0.00 AS dco_debitos,
			   round(case when res_codmon = 'USD' then sum(res_valor) * hpa_tasa_cambio else sum(res_valor) end * cpp_porcentaje / 100.00, 3) AS dco_creditos,
			   hpa_tasa_cambio dco_tasa_cambio,
			   0.00 AS dco_debitos_usd,
			   round(case when res_codmon = 'USD' then sum(res_valor) else sum(res_valor) / hpa_tasa_cambio end * cpp_porcentaje / 100.00, 3) AS dco_creditos_usd,
			   @anio dco_anio,
			   @mes dco_mes,
			   'R' AS dco_tipo_partida,	
			   null dco_codemp,
		...
		FROM sal.res_reservas
			...
			left join eor.cco_centros_de_costo on cco_codigo = cpp_codcco
		WHERE res_codppl = @codppl
		GROUP BY res_codppl, trs_cuenta_aux, trs_codigo, cco_codigo,  trs_descripcion, cco_descripcion, cco_nomenclatura_contable, res_codmon, hpa_tasa_cambio, cpp_porcentaje) r
