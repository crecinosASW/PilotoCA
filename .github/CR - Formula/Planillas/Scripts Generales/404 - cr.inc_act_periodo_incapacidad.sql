...
WHERE emp_codtpl = @codtpl
	...
	AND NOT EXISTS (SELECT NULL
					...
					WHERE din_codixe = ixe_codigo
						AND din_codfin = fin_codigo
						AND din_fecha_inicial = CASE WHEN ixe_inicio < fin_desde THEN fin_desde ELSE ixe_inicio END
						AND din_fecha_final = CASE WHEN ISNULL(ixe_final, DATEADD(DD, 1, fin_hasta)) > fin_hasta THEN fin_hasta ELSE ixe_final END)
	AND sal.empleado_en_gen_planilla(@sessionId, emp_codigo) = 1