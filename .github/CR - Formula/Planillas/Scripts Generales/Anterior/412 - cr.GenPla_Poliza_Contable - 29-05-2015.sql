...
--EXEC cr.GenPla_Poliza_Contable 82
...
BEGIN
    ...
    SELECT inn_codppl AS dco_codppl, 
	       ...
	       round(sum((case when inn_codmon = 'USD' then inn_valor * hpa_tasa_cambio else inn_valor end) * isnull(cpp_porcentaje,100.00) ) / 100.00,2)  AS dco_debitos,
	       0.00 AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   round(sum((case when inn_codmon = 'USD' then inn_valor else inn_valor / hpa_tasa_cambio end) * isnull(cpp_porcentaje,100.00) ) / 100.00,2)  AS dco_debitos_usd,
    ...
    SELECT dss_codppl AS dco_codppl, 
	       ...
	       0.00 AS dco_debitos,
	       round(sum(case when dss_codmon = 'USD' then dss_valor * hpa_tasa_cambio else dss_valor end),2) AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   0.00 dco_debitos_usd,
		   round(sum(case when dss_codmon = 'USD' then dss_valor else dss_valor / hpa_tasa_cambio end),2) AS dco_creditos_usd,
	       @anio dco_anio,
	       @mes dco_mes,
    ...
    SELECT hpa_codppl dco_codppl,
	       ...
	       ROUND((
			   ISNULL((
				   SELECT ROUND(case when tpl_codmon = 'USD' then SUM(dco_debitos_usd) * hpa_tasa_cambio else SUM(dco_debitos) end, 2)
				   FROM #Ingresos
				   WHERE dco_codppl = hpa_codppl
				   GROUP BY dco_codppl), 0.00)
			   - ISNULL((
				   SELECT ROUND(case when tpl_codmon = 'USD' then SUM(dco_creditos_usd) * hpa_tasa_cambio else SUM(dco_creditos) end, 2)
				   FROM #Descuentos
				   WHERE dco_codppl = hpa_codppl
				   GROUP BY dco_codppl), 0.00)), 2) dco_credito,
		   hpa_tasa_cambio dco_tasa_cambio,
	       0.00 dco_debito_usd,
	       ROUND((
			   ISNULL((
				   SELECT ROUND(case when tpl_codmon = 'USD' then SUM(dco_debitos_usd) else SUM(dco_debitos) / hpa_tasa_cambio end, 2)
				   FROM #Ingresos
				   WHERE dco_codppl = hpa_codppl
				   GROUP BY dco_codppl), 0.00)
			   - ISNULL((
				   SELECT ROUND(case when tpl_codmon = 'USD' then SUM(dco_creditos_usd) else SUM(dco_creditos) / hpa_tasa_cambio end, 2)
				   FROM #Descuentos
				   WHERE dco_codppl = hpa_codppl
				   GROUP BY dco_codppl), 0.00)), 2) dco_credito_usd,
	       @anio dco_anio,
	       @mes dco_mes,
    ...
    SELECT res_codppl AS dco_codppl, 
	       ...
	       round(sum((case when res_codmon = 'USD' then res_valor * hpa_tasa_cambio else res_valor end) * isnull(cpp_porcentaje,100) )  / 100.00,2)  AS dco_debitos,
	       0.00 AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   round(sum((case when res_codmon = 'USD' then res_valor else res_valor / hpa_tasa_cambio end) * isnull(cpp_porcentaje,100) )  / 100.00,2)  AS dco_debitos_usd,
	       0.00 AS dco_creditos_usd,
	       @anio dco_anio,
	       @mes dco_mes,
    ...
    SELECT res_codppl AS dco_codppl, 
	       ...
	       0.00  AS dco_debitos,
	       round(sum(case when res_codmon = 'USD' then res_valor * hpa_tasa_cambio else res_valor end),2) AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   0.00  AS dco_debitos_usd,
	       round(sum(case when res_codmon = 'USD' then res_valor else res_valor / hpa_tasa_cambio end),2) AS dco_creditos_usd,
	       @anio dco_anio,
	       @mes dco_mes,
	       identity(int, 10000, 1) AS dco_linea,
	       'R' AS dco_tipo_partida,	
    ...
    FROM sal.res_reservas
        ...
        join sal.trs_tipos_reserva on trs_codigo = res_codtrs
    WHERE res_codppl = @codppl
    GROUP BY res_codppl, trs_cuenta_aux, trs_codigo, trs_descripcion, res_codmon, hpa_tasa_cambio
