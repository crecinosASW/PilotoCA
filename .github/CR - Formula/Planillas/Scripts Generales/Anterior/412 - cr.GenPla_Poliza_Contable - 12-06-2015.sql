...
--EXEC cr.GenPla_Poliza_Contable 1125
...
BEGIN
    ...
    SELECT inn_codppl AS dco_codppl, 
	       ...
	       round(case when inn_codmon = 'USD' then sum(inn_valor) * hpa_tasa_cambio else sum(inn_valor) end, 2)  AS dco_debitos,
	       0.00 AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   round(case when inn_codmon = 'USD' then sum(inn_valor) else sum(inn_valor) / hpa_tasa_cambio end, 2)  AS dco_debitos_usd,
    ...
    SELECT dss_codppl AS dco_codppl, 
	       ...
	       0.00 AS dco_debitos,
	       round(case when dss_codmon = 'USD' then sum(dss_valor) * hpa_tasa_cambio else sum(dss_valor) end, 2) AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   0.00 dco_debitos_usd,
		   round(case when dss_codmon = 'USD' then sum(dss_valor) else sum(dss_valor) / hpa_tasa_cambio end, 2) AS dco_creditos_usd,
	       @anio dco_anio,
	       @mes dco_mes,
    ...
    SELECT hpa_codppl dco_codppl,
	       ...
	       ROUND((
			   ISNULL((
				   SELECT ROUND(SUM(dco_debitos), 2)
				   FROM #Ingresos
				   WHERE dco_codppl = hpa_codppl
				   GROUP BY dco_codppl), 0.00)
			   - ISNULL((
				   SELECT ROUND(SUM(dco_creditos), 2)
				   FROM #Descuentos
				   WHERE dco_codppl = hpa_codppl
	       ...
	       ROUND((
			   ISNULL((
				   SELECT ROUND(SUM(dco_debitos_usd), 2)
				   FROM #Ingresos
				   WHERE dco_codppl = hpa_codppl
				   GROUP BY dco_codppl), 0.00)
			   - ISNULL((
				   SELECT ROUND(SUM(dco_creditos_usd), 2)
				   FROM #Descuentos
				   WHERE dco_codppl = hpa_codppl
    ...
    SELECT res_codppl AS dco_codppl, 
	       ...
	       round(case when res_codmon = 'USD' then sum(res_valor) * hpa_tasa_cambio else sum(res_valor) end, 2)  AS dco_debitos,
	       0.00 AS dco_creditos,
		   hpa_tasa_cambio dco_tasa_cambio,
		   round(case when res_codmon = 'USD' then sum(res_valor) else sum(res_valor) / hpa_tasa_cambio end, 2)  AS dco_debitos_usd,
	       0.00 AS dco_creditos_usd,
	       @anio dco_anio,
	       @mes dco_mes,
	...
	FROM (
		SELECT res_codppl AS dco_codppl, 
			   ...
			   0.00 AS dco_debitos,
			   round(case when res_codmon = 'USD' then sum(res_valor) * hpa_tasa_cambio else sum(res_valor) end, 2) AS dco_creditos,
			   hpa_tasa_cambio dco_tasa_cambio,
			   0.00 AS dco_debitos_usd,
			   round(case when res_codmon = 'USD' then sum(res_valor) else sum(res_valor) / hpa_tasa_cambio end, 2) AS dco_creditos_usd,
			   @anio dco_anio,
			   @mes dco_mes,
			   'R' AS dco_tipo_partida,	
			   null dco_codemp,
