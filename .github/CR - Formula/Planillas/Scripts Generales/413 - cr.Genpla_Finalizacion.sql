IF EXISTS (SELECT NULL
		   FROM sys.objects
		   WHERE object_id = object_id('cr.Genpla_Finalizacion'))
BEGIN
	DROP PROCEDURE cr.Genpla_Finalizacion
END

GO

--EXEC cr.Genpla_Finalizacion NULL, 80, 'admin'
CREATE procedure cr.Genpla_Finalizacion
...
BEGIN
	...
	SELECT @codagr_no_aplican = agr_codigo
	FROM sal.agr_agrupadores
	WHERE agr_codpai = @codpai
		AND agr_abreviatura = 'CRDescuentosNoAplican'
...
--			dbcc checkident('sal.dss_descuentos', reseed, @maxId)
			...
			--Eliminación de descuento que no aplican a la quinta semana
			IF @codtpl_visual = '4' AND @ppl_frecuencia = 5
			BEGIN
				...
				where dss_codppl = @codppl
					...
					and not exists (select null from sal.dag_descuentos_agrupador where dag_codtdc = dss_codtdc and dag_codagr = @codagr_no_aplican)
...
--			dbcc checkident('sal.hpa_hist_periodos_planilla', reseed, @maxId)
        ...
        INSERT INTO sal.hco_hist_cco_periodos_planilla
			...
			IF @tpl_aplicacion = 'Normal'
				EXEC cr.Genpla_Genera_Reservas @codppl, @sessionId

			EXEC cr.GenPla_Poliza_Contable @codppl, @sessionId

			-- Actualización de incapacidades aplicadas en planilla
			UPDATE p
			SET p.pie_aplicado_planilla = 1
			...
			-- Actualización de cuotas de descuentos cíclicos de embargos
			UPDATE c 
			SET cdc_aplicado_planilla = emb_aplicado_planilla,
				cdc_valor_cobrado = emb_valor
			FROM sal.cdc_cuotas_descuento_ciclico c
				JOIN sal.emb_embargos ON cdc_codigo = emb_codcdc
