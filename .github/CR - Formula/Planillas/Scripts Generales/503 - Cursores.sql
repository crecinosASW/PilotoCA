/* Script generated by Evolution - FormulaEditor. 26/02/2015 11:01 p.m. */
...
GROUP BY inn_codppl, inn_codemp','inn_codemp','inn_codppl','TodosExcluyendo',0,0);
...
SELECT @codppl tnn_codppl,
	...
	CASE 
		WHEN tnn_fecha_del <= @fecha_inicio AND tnn_fecha_al >= @fecha_fin THEN
			gen.fn_diff_two_dates_30(@fecha_inicio, @fecha_fin)
		WHEN tnn_fecha_del <= @fecha_inicio AND tnn_fecha_al >= @fecha_inicio AND tnn_fecha_al <= @fecha_fin THEN
			gen.fn_diff_two_dates_30(@fecha_inicio, tnn_fecha_al)
		WHEN tnn_fecha_del >= @fecha_inicio AND tnn_fecha_del <= @fecha_fin AND tnn_fecha_al >= @fecha_fin THEN
			gen.fn_diff_two_dates_30(tnn_fecha_del, @fecha_fin)
		WHEN tnn_fecha_del >= @fecha_inicio AND tnn_fecha_al <= @fecha_fin THEN
			gen.fn_diff_two_dates_30(tnn_fecha_del, tnn_fecha_al)
		ELSE
			0.00
