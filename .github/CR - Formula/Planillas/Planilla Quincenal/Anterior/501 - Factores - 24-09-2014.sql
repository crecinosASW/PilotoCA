...
/* Script generated by Evolution - FormulaEditor. 04/09/2014 08:41 a.m. */
...
valor = Round(Factores("AntipoQuincenal").Value + _
			   Factores("Salario").Value + _        
...
liquido = Factores("TotalIngresos").Value - _
	Factores("DescuentoAnticipo").Value - _
	Factores("DescuentoIncapacidad").Value
...
If Periodos.Fields("ppl_frecuencia").Value = 2 Then
	If Empleados.Fields("aplica_seguro_social").Value Then
		...
		If Not isnull(Factores("SeguroSocial").CodTipoDescuento) And cuota > 0.00 And liquido - cuota >= 0.00 Then       
		 agrega_descuentos_historial Agrupadores, _
									DescuentosEstaPlanilla, _
									Empleados.Fields("emp_codigo").Value, _
									Periodos.Fields("ppl_codigo").Value, _
...
If Periodos.Fields("ppl_frecuencia").Value = 2 Then
	...
	liquido = Factores("TotalIngresos").Value - _    
		...
		Factores("PensionAlimenticiaPorcentaje").Value - _
		Factores("Embargo").Value - _
		Factores("EmbargoPorcentaje").Value
	...
	If Not IsNull(Factores("SeguroMedico").CodTipoDescuento) And valor > 0 And liquido - valor >= 0 Then       
		agrega_descuentos_historial Agrupadores, _
									DescuentosEstaPlanilla, _
									Empleados.Fields("emp_codigo").Value, _
									Periodos.Fields("ppl_codigo").Value, _
		...
		Do Until PrestacionesEmpleado.Eof
			...
			If Not IsNull(PrestacionesEmpleado.Fields("pre_codtig").Value) And valor > 0.00 Then
				agrega_ingresos_historial Agrupadores, _ 
							...
							valor, _
							PrestacionesEmpleado.Fields("ppp_codmon").Value, 0.00, "Dias"
...
If Not (RubrosSalarioHistorial.Bof And RubrosSalarioHistorial.Eof) Then
	...
	Do Until RubrosSalarioHistorial.Eof
		...
		salanterior = salanterior / 30 * CDbl(diasanterior)
...
delete from sal.fac_factores where fac_codigo = 'e87b9c24-5ffd-42b2-93f4-b69460f20f40';

insert into sal.fac_factores (fac_codigo,fac_id,fac_descripcion,fac_vbscript,fac_codfld,fac_codpai,fac_size) values ('e87b9c24-5ffd-42b2-93f4-b69460f20f40','DescuentoIncapacidad','Descuento por incapacidad','Function DescuentoIncapacidad()

valor = 0.00
total = 0.00
liquido = Round(Factores("TotalIngresos").Value - _
	Factores("DescuentoAnticipo").Value, 2)
   ...
   If Incapacidades.RecordCount > 0 Then
      ...
      If Not (Incapacidades.Bof And Incapacidades.Eof) Then 
         ...
         Do While Not Incapacidades.Eof   
            ...
            dias = CDbl(Incapacidades.Fields("pie_dias").Value)
            valor = CDbl(Incapacidades.Fields("pie_valor_a_descontar").Value)

            total = total + valor

	         If Not IsNull(Incapacidades.Fields("txi_codtdc").Value) And valor > 0.00 And liquido - valor >= 0.00 Then
				agrega_descuentos_historial Agrupadores, _
											DescuentosEstaPlanilla, _
											Empleados.Fields("emp_codigo").Value, _
											Periodos.Fields("ppl_codigo").Value, _
											Incapacidades.Fields("txi_codtdc").Value, _
											Round(valor, 2), 0.00, 0.00, RubrosSalario.Fields("ese_codmon").Value, _
											dias, "Dias"
