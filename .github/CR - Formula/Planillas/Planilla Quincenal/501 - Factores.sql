...
/* Script generated by Evolution - FormulaEditor. 26/02/2015 11:02 p.m. */
...
If Not (RubrosSalarioHistorial.Bof And RubrosSalarioHistorial.Eof) Then
	...
	Do Until RubrosSalarioHistorial.Eof
		...
		If (Day(RubrosSalarioHistorial.Fields("ese_fecha_inicio").Value) > 30.00) Or (Month(RubrosSalarioHistorial.Fields("ese_fecha_inicio").Value) = 2.00 And Day(RubrosSalarioHistorial.Fields("ese_fecha_inicio").Value) >= 28.00) Then			
				diasanterior = 30.00 - Day(fechaini)
			Else
		...
		If liquido > 0.00 Then
			If valor > liquido Then
				...
				agrega_descuentos_historial Agrupadores, _
												...
												liquido, 0, 0, DescuentosEventuales.Fields("ods_codmon").Value, 0, "Dias"				
			Else
				liquido = liquido - valor
				total = total + valor
				DescuentosEventuales.Fields("ods_aplicado_planilla").Value = 1
