...
*/
--EXEC rel.procesa_marcas 1, '4', '20150701', NULL, 'admin'
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			WHERE (djo_hora_ini IS NOT NULL AND djo_hora_fin IS NOT NULL)
				--OR djo_dia = 6 
				OR djo_dia = 0
			...
			WHILE @@FETCH_STATUS = 0
			BEGIN
				IF @jor_hora_entrada IS NULL OR @jor_hora_salida IS NULL
				BEGIN
					SELECT @jor_hora_entrada = CONVERT(DATETIME, CONVERT(VARCHAR, @asi_hora_entrada, 103) + ' ' + CONVERT(VARCHAR, MIN(djo_hora_ini), 108)),
						@jor_hora_salida = CONVERT(DATETIME, CONVERT(VARCHAR, @asi_hora_salida, 103) + ' ' + CONVERT(VARCHAR, MAX(djo_hora_fin), 108))
					FROM sal.djo_dias_jornada
					WHERE djo_codjor = @jor_codigo
				END
...
SELECT asi_codemp, asi_codjor, djo_dia + 1 djo_dia, jor_codigo_visual, djo_hora_ini, djo_hora_fin
FROM (
	...
	FROM (
		...
		HAVING COUNT(*) = (SELECT MAX(num_jornadas)
							...
							WHERE a.asi_codemp = s.asi_codemp)
			AND COUNT(*) > 3) i
...
FETCH NEXT FROM descansos INTO @asi_codemp, @asi_codjor, @djo_dia_descanso, @jor_codigo_visual, @jor_hora_entrada, @jor_hora_salida
			...
			BEGIN
				...
				IF EXISTS (SELECT NULL 
						   ...
						   WHERE asi_codemp = @asi_codemp 
							   ...
							   --AND (ABS(DATEDIFF(MI, @jor_hora_entrada, @asi_hora_entrada)) / 60.00 +
								  --  ABS(DATEDIFF(MI, @jor_hora_salida, @asi_hora_salida)) / 60.00) < @horas_aplica_jor) 
					AND @jor_codigo_visual IN ('NOCTURNA', 'NOCTURNACS')
				BEGIN
					UPDATE rel.asi_asistencias
