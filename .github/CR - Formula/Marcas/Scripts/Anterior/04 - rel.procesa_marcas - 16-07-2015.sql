...
BEGIN
	...
	BEGIN
		...
		IF DATEPART(MI, @asi_hora_entrada) <= 30
			SET @asi_hora_entrada_sig = DATEADD(MI, 30, DATEADD(MI, -DATEPART(MI, @asi_hora_entrada), @asi_hora_entrada))
		ELSE
		...
		ELSE
			SET @asi_hora_entrada_ajus = DATEADD(MI, -30, @asi_hora_entrada_sig)

		-- Ajuste de hora de salida
		IF DATEPART(MI, @asi_hora_salida) < 30
			SET @asi_hora_salida_ant = DATEADD(MI, -DATEPART(MI, @asi_hora_salida), @asi_hora_salida)
		ELSE
			SET @asi_hora_salida_ant = DATEADD(MI, 30, DATEADD(MI, -DATEPART(MI, @asi_hora_salida), @asi_hora_salida))

		IF DATEDIFF(MI, @asi_hora_salida_ant, @asi_hora_salida) <= @minutos_ajuste
			SET @asi_hora_salida_ajus = @asi_hora_salida_ant
		ELSE
			SET @asi_hora_salida_ajus = DATEADD(MI, 30, @asi_hora_salida_ant)
...
SELECT asi_codemp, asi_codjor, djo_dia + 1 djo_dia
FROM (
...
FETCH NEXT FROM descansos INTO @asi_codemp, @asi_codjor, @djo_dia_descanso
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF NOT EXISTS (SELECT NULL FROM rel.asi_asistencias WHERE asi_codemp = @asi_codemp AND asi_fecha = @fecha_act)
				BEGIN
					INSERT INTO rel.asi_asistencias (
						asi_codemp,
						asi_codppl,
